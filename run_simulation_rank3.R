...
dset    <-  1  # index for the dth dataset (any number between 1-150)
  V_size  <-  3203  # 3203: U3 V=20       3703: U3 V=70        4203: U4 V=20        4703: U4 V=70
  
  if(V_size == 3203){
    load("./rank3/U3_V20_rank3.rdata")
    V  = 20
    
  }else if(V_size == 3703){
    load("./rank3/U3_V70_rank3.rdata")
    V = 70
    
  }else if(V_size == 4203){
    load("./rank3/U4_V20_rank3.rdata")
    V = 20
    
  }else if(V_size == 4703){
    load("./rank3/U4_V70_rank3.rdata")
    V = 70
...
out   <- A_standard3(X, U1, U2, U3, V, top)
C     <- out[[2]]
W     <- nrow(C)
...
#-------------------------------------------------------------------------------------
# Test
out    <- A_standard3(X_test, U1, U2, U3, V, top)
C      <- out[[2]]
E      <- out[[3]]
W      <- nrow(C)
...
origin <- U1[,C[,1]]*U1[,C[,2]]  +  U2[,C[,1]]*U2[,C[,2]]  + U3[,C[,1]]*U3[,C[,2]]
origin <- t(origin)     # dim = pairs x SNP
...
print(a)
#save(a, file = paste0("a3_", V_size, "_", sigma2, "_", dset, ".rdata"))  
