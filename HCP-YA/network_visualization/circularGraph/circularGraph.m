...
% Copyright 2016 The MathWorks, Inc.
  ...
  methods
    function this = circularGraph(adjacencyMatrix,varargin)
      ...
      edgecolormaprange = [min(min(adjacencyMatrix)),max(max(adjacencyMatrix)),];
      ...
      extent = zeros(length(adjacencyMatrix),1);
       ...
       t2 = linspace(pi/2-dpi/2,-pi/2+dpi/2,N_half_circ);
       t = [t1,t2]';
      ...
      MyEdgeColormap=jet(ceil(maxv*100)+1);
      
      for i=1:length(edge_weigth)
          newedge_weigth(i) = min(ceil(maxv*100/2+1) + ceil((edge_weigth(i)-this.EdgeColorMapRange(1))*100),ceil(maxv*100)+1);
      end
