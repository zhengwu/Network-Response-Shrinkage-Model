  ...
  methods
    function this = circularGraph(adjacencyMatrix,varargin)
      ...
      for i = 1:length(adjacencyMatrix)/2
        this.Node(i) = node(r*cos(t(i))-d_half_dist,r*sin(t(i)));
        this.Node(i).Color = this.ColorMap(i,:);
        this.Node(i).Label = this.Label{i};
      end
      
      for i = (length(adjacencyMatrix)/2 + 1):length(adjacencyMatrix)
        this.Node(i) = node(r*cos(t(i))+d_half_dist,r*sin(t(i)));
        this.Node(i).Color = this.ColorMap(i,:);
        this.Node(i).Label = this.Label{i};
      end
      ...
      for i = 1:length(v)
        if row(i) ~= col(i)
          if abs(row(i) - col(i)) - length(adjacencyMatrix)/2 == 0 
            ...
            if(row(i)>(length(adjacencyMatrix)/2+0.5))
                u  = [r*cos(t(row(i)))+d_half_dist;r*sin(t(row(i)))];
            else
                u  = [r*cos(t(row(i)))-d_half_dist;r*sin(t(row(i)))];
            end
            
            if(col(i)>(length(adjacencyMatrix)/2+0.5))
                v  = [r*cos(t(col(i)))+d_half_dist;r*sin(t(col(i)))];
            else
                v  = [r*cos(t(col(i)))-d_half_dist;r*sin(t(col(i)))];
            end
            ...
            this.Node(row(i)).Connection(end+1) = line(...
              ...
              if(row(i)>(length(adjacencyMatrix)/2+0.5))
                  u  = [r*cos(t(row(i)))+d_half_dist;r*sin(t(row(i)))];
                  u_rconn = [cos(t(row(i)));sin(t(row(i)))];
              else
                  u  = [r*cos(t(row(i)))-d_half_dist;r*sin(t(row(i)))];
                  u_rconn = [cos(t(row(i)));sin(t(row(i)))];
              end
              ...
              if(col(i)>(length(adjacencyMatrix)/2+0.5))
                  v  = [r*cos(t(col(i)))+d_half_dist;r*sin(t(col(i)))];
                  v_rconn = [cos(t(col(i)));sin(t(col(i)))];
              else
                  v  = [r*cos(t(col(i)))-d_half_dist;r*sin(t(col(i)))];
                  v_rconn = [cos(t(col(i)));sin(t(col(i)))];
              end
              
            x0 = -(u_rconn(2)-v_rconn(2))/(u_rconn(1)*v_rconn(2)-u_rconn(2)*v_rconn(1));
            y0 =  (u_rconn(1)-v_rconn(1))/(u_rconn(1)*v_rconn(2)-u_rconn(2)*v_rconn(1));
            r_conn  = sqrt(x0^2 + y0^2 - 1);

            thetaLim(1) = atan2(u_rconn(2)-y0,u_rconn(1)-x0);
            thetaLim(2) = atan2(v_rconn(2)-y0,v_rconn(1)-x0);
